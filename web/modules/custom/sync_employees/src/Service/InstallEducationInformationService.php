...
class InstallEducationInformationService {
    ...
    public function deleteProfiles(): void {
        ...
        $ids = $storage->getQuery()
            ->condition('type', 'education_information')
            ->accessCheck(FALSE)
            ->execute();
        if (!empty($ids)) {
    ...
    public function deleteFieldConfig(): void {
        foreach ($this->fields as $field_name) {
            $instance = FieldConfig::loadByName('profile', 'education_information', $field_name);
            if ($instance) {
                $instance->delete();
            }
        }
    ...
    public function deleteEntityFormDisplay(): void {
        $form_display_id = 'profile.education_information.default';
    ...
    public function deleteEntityViewDisplay(): void {
        $view_display_id = 'profile.education_information.default';
    ...
    public function deleteProfileType(): void {
        ...
        $profile_type = $profile_type_storage->load('education_information');
        if ($profile_type) {
            $profile_type->delete();
        }
    }
