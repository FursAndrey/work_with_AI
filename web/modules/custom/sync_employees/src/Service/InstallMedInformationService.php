...
class InstallMedInformationService {
    ...
    public function deleteProfiles(): void {
        ...
        $ids = $storage->getQuery()
            ->condition('type', 'med_information')
            ->accessCheck(FALSE)
            ->execute();
        if (!empty($ids)) {
    ...
    public function deleteFieldConfig(): void {
        foreach ($this->fields as $field_name) {
            $instance = FieldConfig::loadByName('profile', 'med_information', $field_name);
            if ($instance) {
                $instance->delete();
            }
        }
    ...
    public function deleteEntityFormDisplay(): void {
        $form_display_id = 'profile.med_information.default';
    ...
    public function deleteEntityViewDisplay(): void {
        $view_display_id = 'profile.med_information.default';
    ...
    public function deleteProfileType(): void {
        ...
        $profile_type = $profile_type_storage->load('med_information');
        if ($profile_type) {
            $profile_type->delete();
        }
    }
