...
class InstallViolationInformationService {
    ...
    public function deleteProfiles(): void {
        ...
        $ids = $storage->getQuery()
            ->condition('type', 'violation_information')
            ->accessCheck(FALSE)
            ->execute();
        if (!empty($ids)) {
    ...
    public function deleteFieldConfig(): void {
        foreach ($this->fields as $field_name) {
            $instance = FieldConfig::loadByName('profile', 'violation_information', $field_name);
            if ($instance) {
                $instance->delete();
            }
        }
    ...
    public function deleteEntityFormDisplay(): void {
        $form_display_id = 'profile.violation_information.default';
    ...
    public function deleteEntityViewDisplay(): void {
        $view_display_id = 'profile.violation_information.default';
    ...
    public function deleteProfileType(): void {
        ...
        $profile_type = $profile_type_storage->load('violation_information');
        if ($profile_type) {
            $profile_type->delete();
        }
    }
