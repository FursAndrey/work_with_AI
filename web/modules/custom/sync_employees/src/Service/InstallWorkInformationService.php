 ...
 */
class InstallWorkInformationService {
    private $fields = [
        'field_work_number',
        'field_work_unit',
        'field_work_position',
        'field_work_position_type',
        'field_work_enterdate',
        'field_work_fireddate',
    ];
    ...
    public function deleteProfiles(): void {
        ...
        $ids = $storage->getQuery()
            ->condition('type', 'work_information')
            ->accessCheck(FALSE)
            ->execute();
        if (!empty($ids)) {
    ...
    public function deleteFieldConfig(): void {
        foreach ($this->fields as $field_name) {
            $instance = FieldConfig::loadByName('profile', 'work_information', $field_name);
            if ($instance) {
                $instance->delete();
            }
        }
    ...
    public function deleteEntityFormDisplay(): void {
        $form_display_id = 'profile.work_information.default';
    ...
    public function deleteEntityViewDisplay(): void {
        $view_display_id = 'profile.work_information.default';
    ...
    public function deleteProfileType(): void {
        ...
        $profile_type = $profile_type_storage->load('work_information');
        if ($profile_type) {
            $profile_type->delete();
        }
    }
